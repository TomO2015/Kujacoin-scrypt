...
#if defined(NDEBUG)
# error "Kuja cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x92954257f2d18e605a3d9d701ce096123795cf7ecc4f72dd4811c5474444abb7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kuja: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kuja Signed Message:\n";
...
{
    // Kuja: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 876000); // Kuja: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Kuja: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Kuja: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kuja: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kuja: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd1d0464556773910d479f08651968a387b01e83eb18cb0a4bb4a62019ea644b6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bideo Netanyahu divisions burst out into the open again 11th March 24";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043a8da00c14a25e9c92902da7bfe8f71fbab4f4e4f5f53f167122f65187907b3a5e81acfe789203a4e2b49666838d041a20f29b1b023b6219089b5e9124eeb21a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1710174199;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085376946;

        if (fTestNet)
        {
            block.nTime    = 1710174174;
            block.nNonce   = 385703001;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x05b0cbd00d537ba790904943159f7e0e0568e81cbba99fbf1c307899812b3ceb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xbc, 0xd6 }; // Kuja: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KujaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KujaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KujaMiner(CWallet *pwallet)
{
    printf("KujaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kuja-miner");
    ...
    try { loop {
        ...
        printf("Running KujaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KujaMiner, pwallet));
}
